...
本文 SQL 草案中使用 `CURRENT_TIMESTAMP` 作为数据库默认值。实际实现时，应用层也可以显式写入 UTC ISO-8601 时间。
...
| `created_at` | `TEXT` | 否 | `CURRENT_TIMESTAMP` | 记录创建时间。 |
| `updated_at` | `TEXT` | 否 | `CURRENT_TIMESTAMP` | 记录最近更新时间。 |
...
| `imported_at` | `TEXT` | 否 | `CURRENT_TIMESTAMP` | 文件导入 StudySpace 的时间。 |
| `created_at` | `TEXT` | 否 | `CURRENT_TIMESTAMP` | 记录创建时间，通常与 `imported_at` 一致。 |
| `updated_at` | `TEXT` | 否 | `CURRENT_TIMESTAMP` | 元数据最近更新时间。该字段变化不代表原文件被修改。 |
...
| `created_at` | `TEXT` | 否 | `CURRENT_TIMESTAMP` | 记录创建时间。 |
| `updated_at` | `TEXT` | 否 | `CURRENT_TIMESTAMP` | 设置最近更新时间。 |
...
* 正式发布前应加入 schema 版本表，例如 `schema_migrations`。
